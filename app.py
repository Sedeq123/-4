...
# ==========================================
# الألوان: أسود ملكي (#0B0B0B)، كحلي داكن (#0F172A)، ذهبي فاخر (#D4AF37)، أبيض عاجي (#FFFFF0)
st.markdown("""
<style>
    ...
    /* صناديق الإحصائيات والمقاييس (Metrics) */
...
# ==========================================
# 3. قواعد البيانات الثابتة والأسعار
# ==========================================
prices = {
    "بوليستر": 80,
    "نايلون": 120,
...
with st.sidebar:
    ...
    # تم ربط الخيارات بمفتاح تحكم موحد مأخوذ من رؤيتك المحدثة
    page = st.radio(
        "لوحة التحكم والمنصة",
        [
            "🏠 الرئيسية",
...
if page == "🏠 الرئيسية":
    ...
    # أزرار التنقل السريع الافتراضية
    col_b1, col_b2, col_b3, col_b4 = st.columns(4)
    with col_b1:
        if st.button("🚀 ابدأ التصميم الآن"):
            st.info("فضلاً اختر '🎨 استوديو التصميم الذكي' من القائمة الجانبية")
    with col_b2:
        if st.button("📚 استعرض الكاتالوج"):
            st.info("فضلاً اختر '📚 الكاتالوج الفاخر' من القائمة الجانبية")
    with col_b3:
        if st.button("💰 احسب السعر فوراً"):
            st.info("فضلاً اختر '💰 التسعير الفوري' من القائمة الجانبية")
    with col_b4:
        if st.button("📞 تواصل معنا"):
            st.info("فضلاً اختر '📞 التواصل والطلب' من القائمة الجانبية")
...
# ==========================================
    ...
    if st.button("🚀 إنشاء وتوليد التصميم"):
        ...
        # عملية تسعير ذكية مدمجة في صفحة التصميم
        area = width * height
        cost = area * prices[material]
        
        st.markdown(f"""
        <div style="background: #111827; padding: 25px; border-radius:15px; border: 1px solid #22C55E;">
            ...
            <p style="font-size:12px; color:#aaa;">*هذا السعر يشمل الفحص الفني الأولي وإرسال الطلب مباشرة لخطوط المصنع.</p>
        </div>
        """, unsafe_allow_html=True)
...
# ==========================================
    ...
    st.write("نعتمد في استوديو نسيج على أرقى وأجود أنواع الخيوط المجهزة لتحمل الاستخدام الطويل والحفاظ على بريق الألوان.")
    st.markdown("---")
...
# ==========================================
    ...
    st.write("تصفح مجموعاتنا الجاهزة والمصنفة للطلب الفوري.")
    st.markdown("---")
    ...
    with tab_royal:
        st.subheader("التصاميم الفاخرة المخصصة للقصور والعملاء المميزين")
        
        # عرض التصاميم الـ 6 بالتفصيل
    ...
    with tab_home:
        st.subheader("الفئة الأكثر مبيعاً - لمسة دافئة وعصرية لمنزلك")
    ...
    with tab_kids:
        st.subheader("مرحلة إنتاجية مستقبلية: غرف ألعاب وأطفال مبهجة")
        st.markdown("""
    ...
    with tab_corporate:
        st.subheader("مخصصة للفنادق، المكاتب، والشركات الكبرى")
        st.markdown("""
        - **هندسي فاخر:** خطوط مستقيمة حديثة وألوان رسمية (رمادي، كحلي، أسود) تعكس الاحترافية.
...
# ==========================================
    ...
    st.write("احسب تكلفة سجادتك المخصصة فوراً وبكل شفافية بناءً على أبعاد الأمتار ونوع الخامة.")
    st.markdown("---")
    ...
    with col_res:
        ...
        # الحسابات الرياضية المعتمدة في خطتك
        ...
        st.metric(label="السعر النهائي الإجمالي (شامل الدعم الفني)", value=f"{calc_total:,.2f} ريال سعودي")
        
        # مثال توضيحي محاكي لسهولة الفهم المالي للعميل
        st.markdown(f"""
        <div style="background: #0F172A; padding: 15px; border-radius: 10px; border-left: 4px solid #D4AF37; margin-top: 15px;">
            💡 <b>طريقة الحساب الثابتة:</b><br>
            {u_width} متر (عرض) × {u_height} متر (طول) = {calc_area:.1f} متر مربع.<br>
            {calc_area:.1f} م² × {price_per_meter} ريال (خامة {selected_mat}) = <b>{calc_total:,.2f} ريال سعودي</b>.
        </div>
        """, unsafe_allow_html=True)
...
# ==========================================
    ...
    st.write("استعرض جودة التنفيذ الفعلي من داخل خطوط الإنتاج والمجالس الفاخرة لعملائنا.")
    st.markdown("---")
    ...
    with col_proj2:
        ...
        <div class="product-card">
            ...
            <p><b>التنفيذ:</b> نمط "نور المحراب" بخا
